...
func delegateGetReparsePointByName(
	fileSystem, context uintptr, fileName uintptr,
...
func delegateResolveReparsePoints(
	...
	ioStatus uintptr, buffer uintptr, size *uintptr,
) windows.NTStatus {
	// Call the WinFSP API
...
func PosixMapSecurityDescriptorToPermissions(securityDescriptor *windows.SECURITY_DESCRIPTOR) (uid, gid, mode uint32, err error) {
	...
	if callErr == nil && status != windows.STATUS_SUCCESS {
		callErr = status
	}
	if callErr != nil {
...
func PosixMapSidToUid(sid *windows.SID) (uint32, error) {
	...
	if err == nil && status != windows.STATUS_SUCCESS {
		err = status
	}
	if err != nil {
...
func FileSystemFindReparsePoint(
	...
	return result != 0, reparsePointIndex, nil
}
